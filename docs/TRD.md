...
- **Max-turn cap (DRAW termination).** `GameConfig.max_turns` is a hard upper bound on `state.attempt_count` — i.e., the total number of `step()` calls (legal or illegal). When `attempt_count` reaches `max_turns` without a winner, the engine transitions to `Status.DRAW` and rejects further actions with `IllegalReason.GAME_OVER`. `Status` therefore has three values: `IN_PROGRESS`, `WON`, `DRAW`. `build_two_player_config(num_disks)` sets a generous default of `max_turns = 1000` (overridable); construction rejects `max_turns <= 0`. There is no `None` / "unlimited" mode — the spec demands a cap, and forcing an explicit number keeps RL termination guarantees crisp.
